	...
	return (
		...
		>
			...
			>
				...
				/>
				<input
					className="input input-with-icon input-with-kbd"
					placeholder="Buscar..."
					style={{ fontSize: 12 }}
					onChange={(e) => onSearch?.(e.target.value)}
					// CONCEITO: Optional chaining (?.)
					// onSearch pode ser undefined (não passado como prop).
					// onSearch?.() é equivalente a: if (onSearch) onSearch(...)
					// Mais seguro que onSearch && onSearch(...)
				/>
				<kbd
					className="kbd-hint"
					style={{
			...
			<div className="topbar-actions" style={{ display: "flex", gap: 10 }}>
				...
				<button className="btn-primary">
					<Ic.Plus size={14} />
					<span className="topbar-newbtn-label">Novo</span>
				</button>
			</div>
		</header>
	);
